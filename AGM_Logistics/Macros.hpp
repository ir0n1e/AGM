...
#define MACRO_PARADROP \
	class AGM_DropCargo { \
		...
		condition = "getNumber(configFile >> 'CfgVehicles' >> typeOf vehicle _player >> 'AGM_Paradrop') == 1 && {getPosATL vehicle _player select 2 > 60} && {count (vehicle player getVariable ['AGM_Logistics_loadedItems', []]) > 0} && {driver vehicle player == player;}"; \
